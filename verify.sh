#!/bin/sh
# Post-restart verification for dsh-codex-clone (run after the server restarts).
BASE="${1:-http://127.0.0.1:3080}"
echo "== 1. plugin client bundle served =="
curl -s -o /dev/null -w "%{http_code} %{size_download} bytes\n" "$BASE/plugins/dsh-codex-clone/client.js"
...
curl -s "$BASE/" | grep -o "dsh-codex-clone" | head -1
