...
export const NAMESPACE = 'codex-clone'
...
export const name = 'dsh-codex-clone'
...
export function apply(ctx) {
	ctx.inject(['settings'], (settingsCtx) => {
		try {
			...
			settingsCtx.logger?.warn?.(`codex-clone: settings registration failed: ${String(error?.message ?? error)}`)
		}
	})
	ctx.inject(['webServer'], (httpCtx) => {
		httpCtx.effect(() => httpCtx.webServer.register({ kind: 'prefix', path: '/__codex', handler: makeHandler(httpCtx) }), 'codex-clone: /__codex routes')
