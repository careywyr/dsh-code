 ...
 *      GET  /__codex/git-diff?file=<p>   old/new content of one changed file (diff view)
 *      GET  /__codex/version             running DeepSeek Harness (@deepseek-ai/dsh) version
...
/**
 * Resolve the running DeepSeek Harness version (the `@deepseek-ai/dsh`
 * umbrella package). Computed once; the version cannot change mid-process.
 *
 * The goal is the version of the dsh process actually serving right now, so
 * the primary anchor is this process's own entry script. Strategy order:
 *  1. The dsh host entry script (`process.argv[1]`) — it sits inside the
 *     install tree this process was launched from, which is authoritative for
 *     "the running version" even when several npx trees coexist.
 *  2. The install tree this plugin's dependencies are linked into (a sibling
 *     `@deepseek-ai/dsh-settings` resolves there) — a fallback for non-CLI
 *     launches; note install.mjs repoints these links at the newest tree, so
 *     they can drift from the running tree and are only second best.
 *  3. The cwd, as a last resort.
 */
let harnessVersionCache
function readDshVersionInNodeModules(nodeModulesDir) {
	const pkgPath = path.join(nodeModulesDir, '@deepseek-ai', 'dsh', 'package.json')
...
/** Strategy 1: resolve a sibling dependency, walk up to its node_modules root. */
function dshVersionViaSiblingDep() {
	try {
		const req = createRequire(import.meta.url)
		const depEntry = req.resolve('@deepseek-ai/dsh-settings')
		let cur = path.dirname(realpathSync(depEntry))
		for (let depth = 0; depth < 40; depth += 1) {
			if (path.basename(cur) === 'node_modules') return readDshVersionInNodeModules(cur)
			const parent = path.dirname(cur)
			if (parent === cur) break
			cur = parent
		}
	} catch {
		/* dependency link missing — fall through to the other strategies */
	}
	return ''
}
/** Strategies 2/3: walk upward from an anchor until the dsh package.json. */
function dshVersionFromAnchor(anchor) {
	let dir
	try {
		dir = realpathSync(anchor)
	} catch {
		dir = anchor
	}
	let cur = dir
...
function resolveHarnessVersion() {
	if (harnessVersionCache !== undefined) return harnessVersionCache
	// 1) authoritative: this process's own entry script (the running tree).
	let version = ''
	if (typeof process.argv[1] === 'string' && process.argv[1] !== '') {
		version = dshVersionFromAnchor(process.argv[1])
	}
	// 2) fallback: the tree this plugin's dependencies are linked into.
	if (version === '') version = dshVersionViaSiblingDep()
	// 3) last resort: the cwd.
	if (version === '') version = dshVersionFromAnchor(process.cwd())
	harnessVersionCache = version
	return version
}
...
// ── per-file git diff (for the IDEA-style two-pane comparison) ──────────────

/** Diff content reads cap out here; larger files report their size only. */
const DIFF_READ_MAX_TEXT = 1.5 * 1024 * 1024

/** Derive the change kind from the porcelain status + which sides exist. */
function gitDiffKind(statusLine, hasOld, hasNew) {
	if (statusLine.startsWith('??')) return 'untracked'
...
/**
 * Return both sides of one file's working-tree change so the browser can draw
 * a side-by-side comparison. The old side is `HEAD:<file>` (falling back to
 * the index for fresh repos without commits); the new side is the file on
 * disk. This mirrors `git diff HEAD`, the same scope the git card stats use.
 * The target must stay inside the workspace (no `~/` expansion here).
 */
async function gitFileDiff(ctx, url) {
	...
	const probe = await git(['rev-parse', '--is-inside-work-tree'], base)
	if (!probe.ok || probe.stdout.trim() !== 'true') {
		return { status: 400, body: { error: 'not a git repository', path: abs } }
	}
	const relPosix = rel.split(path.sep).join('/')
	// Porcelain status for the change kind (untracked vs added vs modified…).
	const statusR = await git(['status', '--porcelain', '--', relPosix], base)
	const statusLine = statusR.ok ? (statusR.stdout.split('\n').find((l) => l.trim() !== '') ?? '') : ''
	// Numstat for this one file: '-' on either side marks binary content.
	let added = 0
	let deleted = 0
	let binary = false
	const numstatR = await git(['diff', 'HEAD', '--numstat', '--', relPosix], base)
	const numstatSource = numstatR.ok ? numstatR.stdout : (await git(['diff', '--cached', '--numstat', '--', relPosix], base)).stdout ?? ''
	const numstatLine = numstatSource.split('\n').find((l) => l.trim() !== '') ?? ''
	if (numstatLine !== '') {
		const [a, d] = numstatLine.split('\t')
		if (a === '-' || d === '-') binary = true
		else {
			added = Number(a) || 0
			deleted = Number(d) || 0
	...
	// New side: the working-tree file (absent when deleted).
	let newExists = false
	let newContent = null
	let newSize = 0
	try {
		...
		if (st.isFile()) {
			newExists = true
			newSize = st.size
			if (st.size <= DIFF_READ_MAX_TEXT) {
				const buffer = await readFile(abs)
				if (looksBinary(buffer)) binary = true
				else newContent = buffer.toString('utf8')
			}
		}
	} catch {
		/* file absent on disk (deleted) */
	}
	if (newExists && newSize > DIFF_READ_MAX_TEXT) {
		return {
			status: 200,
			body: {
				path: abs, file: relPosix, kind: gitDiffKind(statusLine, true, true),
				tooLarge: true, size: newSize, added, deleted,
			},
		}
	}
	// Old side: HEAD version, or the staged index version when HEAD is missing.
	let oldContent = null
	if (!binary) {
		const headShow = await git(['show', `HEAD:${relPosix}`], base)
		if (headShow.ok) oldContent = headShow.stdout
		else {
			const indexShow = await git(['show', `:${relPosix}`], base)
			if (indexShow.ok) oldContent = indexShow.stdout
		}
	}
	const kind = gitDiffKind(statusLine, oldContent !== null, newExists)
	return {
		status: 200,
		body: {
			path: abs,
...
function makeHandler(ctx) {
	return async (req, res) => {
		try {
			...
			if (route === '/__codex/git-diff') {
				const { status, body } = await gitFileDiff(ctx, url)
				sendJson(res, status, body)
				return
			}
			if (route === '/__codex/version') {
				sendJson(res, 200, { version: resolveHarnessVersion() })
				return
			}
...
export const __harnessVersion = { resolveHarnessVersion, dshVersionFromAnchor }
export const __gitDiff = { gitFileDiff, gitDiffKind }
